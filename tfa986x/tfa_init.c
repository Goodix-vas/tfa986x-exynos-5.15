...
static enum tfa98xx_error tfa9865_specific(struct tfa_device *tfa)
{
	...
	case 0x0a65: /* Initial revision ID TFA9865 N1A0 */
		/* ----- generated code start ----- */
		/* -----  version 12 ----- */
		reg_write(tfa, 0x00, 0xca41); /* POR=0xc241 */
		reg_write(tfa, 0x02, 0x0628); /* POR=0x0008 */
		reg_write(tfa, 0x08, 0x0002); /* POR=0x0000 */
		reg_write(tfa, 0x52, 0x1400); /* POR=0x1140 */
		reg_write(tfa, 0x59, 0x0001); /* POR=0x0000 */
		reg_write(tfa, 0x5a, 0x36bc); /* POR=0x36be */
		reg_write(tfa, 0x5f, 0x0080); /* POR=0x00c0 */
		reg_write(tfa, 0x75, 0x9900); /* POR=0x9200 */
		reg_write(tfa, 0xd2, 0x0000); /* POR=0x03c0 */
		reg_write(tfa, 0xda, 0x0002); /* POR=0x0000	 */
		/* ----- generated code end   ----- */
		break;
	...
	case 0x0c65: /* TFA9865 N1C0 */
		...
		reg_write(tfa, 0x50, 0xc000); /* POR=0x8000 */
		reg_write(tfa, 0x62, 0x0580); /* POR=0x0582 */
		reg_write(tfa, 0x63, 0x0194); /* POR=0x0182 */
		reg_write(tfa, 0x74, 0x61a0); /* POR=0x50f0 */
		reg_write(tfa, 0x75, 0x9a00); /* POR=0x9200 */
		reg_write(tfa, 0x78, 0x0001); /* POR=0x000d */
		reg_write(tfa, 0xd7, 0x0001); /* POR=0x0000 */
		reg_write(tfa, 0xdd, 0x0006); /* POR=0x005e */
		/* ----- generated code end   ----- */
		break;
...
static int tfa9865_set_swprofile(struct tfa_device *tfa,
	unsigned short new_value)
{
	enum tfa98xx_error err = TFA98XX_ERROR_OK;
	int active_value = tfa_dev_get_swprof(tfa);
...
static int tfa9865_set_swvstep(struct tfa_device *tfa,
	unsigned short new_value)
{
	/* Set the new value in the struct */
	tfa->vstep = new_value - 1;
 ...
 *  return: *ready = 1 when clocks are stable to allow DSP subsystem access
 */
static enum tfa98xx_error tfa9865_dsp_system_stable(struct tfa_device *tfa,
	int *ready)
{
	enum tfa98xx_error error = TFA98XX_ERROR_OK;
...
static int tfa9865_set_bitfield(struct tfa_device *tfa,
	uint16_t bitfield, uint16_t value)
{
	uint16_t reg = (bitfield >> 8) & 0xff;
...
void tfa9865_ops(struct tfa_device_ops *ops)
{
	...
	ops->set_mute = tfa_set_mute_nodsp;
	ops->set_bitfield = tfa9865_set_bitfield;
}
